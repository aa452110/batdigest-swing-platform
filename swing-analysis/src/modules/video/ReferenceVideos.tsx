...
// No default videos - coaches must add their own
...
const ReferenceVideos: React.FC<ReferenceVideosProps> = ({ onSelectVideo }) => {
  ...
  const [videoType, setVideoType] = useState<'reference' | 'drill'>('reference');
  ...
  const handleAddVideo = () => {
    ...
    setNewVideoName('');
    setHasLocalVideos(true);
  };

  const handleDeleteVideo = (videoName: string) => {
    const video = referenceVideos.find(v => v.name === videoName);
    if (video) {
      ...
      // Remove from state
      const updatedVideos = referenceVideos.filter(v => v.name !== videoName);
      setReferenceVideos(updatedVideos);
      
      // Clear delete confirmation
      setDeleteConfirm(null);
      
      // Update hasLocalVideos flag
    ...
    <>
      <div className="bg-gray-800 rounded-lg p-4">
        <div className="flex justify-between items-center mb-3">
          <h3 className="text-sm font-semibold text-gray-300">Reference Videos</h3>
          <button 
    ...
    {showAddModal && (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
          <h2 className="text-xl font-bold text-white mb-4">Add Reference Video</h2>
